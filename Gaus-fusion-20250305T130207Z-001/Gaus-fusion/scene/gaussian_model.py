...
class GaussianModel(nn.Module):
    ...
    def training_setup(self, training_args):
        ...
        if self.optimizer_type == "default":
            ...
            try:
                self.optimizer = SparseGaussianAdam(l, eps=1e-15)
            except:
                # A special version of the rasterizer is required to enable sparse adam
                self.optimizer = torch.optim.Adam(l, eps=1e-15)

        # Yaniv --------------------------------------------------------------------------------------------------------
        # print("is optimizer none? answer:", self.optimizer is None)
    ...
    def replace_tensor_to_optimizer(self, tensor, name):
        ...
        for group in self.optimizer.param_groups:
            if group["name"] == name:
                ...
                # Yaniv ------------------------------------------------------------------------------------------------
                # print("is optimizer none? answer:", self.optimizer is None)
                # print("is stored_state none? answer:", stored_state is None)

                if stored_state is not None:  # Only modify state if it exists
                ...
                if stored_state is not None:  # Restore state only if it existed
    ...
    def print_gaussian_stats(self):
        ...
        avg_scaling = self._scaling.mean().item()
        avg_opacity = self._opacity.mean().item()
        avg_color_dc = self._features_dc.mean().item()
        avg_color_rest = self._features_rest.mean().item()

        print(f"Number of Gaussians: {num_gaussians}")
        print(f"Average Scaling: {avg_scaling:.6f}")
