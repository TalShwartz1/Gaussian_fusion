...
class GaussianDataset:
    def __init__(self, scene, device, type='train', H=453, W=460, size=100):
        ...
        # Load all training cameras
        self.viewpoint_stack = scene.getTrainCameras().copy()
        self.viewpoint_indices = list(range(len(self.viewpoint_stack)))

    def __len__(self):
        return self.size  # Control the dataset length

    def collate(self, index):
        # Always return 1 sample per batch
        if not self.viewpoint_stack:
            self.viewpoint_stack = self.scene.getTrainCameras().copy()
            self.viewpoint_indices = list(range(len(self.viewpoint_stack)))

        rand_idx = np.random.randint(0, len(self.viewpoint_indices))
        viewpoint_cam = self.viewpoint_stack.pop(rand_idx)
        viewpoint_indices = self.viewpoint_indices.pop(rand_idx)

        data = {
            'H': self.H,
            'W': self.W,
            'viewpoint_cam': viewpoint_cam,
            'viewpoint_index': viewpoint_indices,
        }
        return data

    def dataloader(self):
        return DataLoader(
            list(range(len(self.scene.getTrainCameras()))),
            batch_size=1,
