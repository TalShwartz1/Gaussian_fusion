...
class StableDiffusion(nn.Module):
    ...
    # yaniv - changed train_step of stable diffusion -------------------------------------------------------------------
    def train_step(self, text_embeddings, inputs, guidance_scale=100):
        # yaniv - guidance_scale=100 in the original -------------------------------------------------------------------
        # interp to 512x512 to be fed into vae.
        ...
        if not self.latent_mode:
            ...
            #   Yaniv - change 512x512 to 389x260 ----------------------------------------------------------------------
            #     pred_rgb_512 = F.interpolate(inputs, (512, 512), mode='bilinear', align_corners=False)

            pred_rgb_512 = F.interpolate(inputs, (453, 460), mode='bilinear', align_corners=False)
            # yaniv - remove encoding to latent space(removed) ---------------------------------------------------------
            latents = self.encode_imgs(pred_rgb_512)
        ...
        # yaniv - commented out torch.no_grad() --------------------------------------------------------------------
        with torch.no_grad():
            # add noise
            noise = torch.randn_like(latents)
            latents_noisy = self.scheduler.add_noise(latents, noise, t)
        ...
        # noise_pred.requires_grad_()
        # l2_loss = torch.mean((noise_pred - noise) ** 2) * 1000000000
        # l2_loss.backward(retain_graph=True)


        # grad = torch.sign(grad) * torch.abs(grad) ** 2
#         grad = torch.sign(grad) * torch.where(
#             torch.abs(grad) < 1,
#             torch.abs(grad) ** (1/5),  # For values < 1, take sqrt
#             torch.abs(grad) ** 5          # For values >= 1, square
# )

        # print("max gradient = ", grad.min())
        ...
        # yaniv - changed so it will return the loss of diffusion ------------------------------------------------------
        return 0  # dummy loss value

        # loss = grad.norm().item() / (4*32*48) * 10
        # return loss
...
if __name__ == '__main__':
    ...
    # parser.add_argument('-H', type=int, default=512)
    # parser.add_argument('-W', type=int, default=512)
    parser.add_argument('-H', type=int, default=453)
    parser.add_argument('-W', type=int, default=460)
